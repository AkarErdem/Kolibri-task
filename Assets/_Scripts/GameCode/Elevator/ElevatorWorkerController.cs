...
namespace GameCode.Elevator
{
    public class ElevatorWorkerController
    {
        ...
        private readonly MineshaftMvcCollection _mineshaftMvcCollection;
        ...
        public ElevatorWorkerController(ElevatorView elevatorView, ElevatorModel elevatorModel, WorkerModel workerModel,
             MineshaftMvcCollection mineshaftMvcCollection, CompositeDisposable disposable)
        {
            _elevatorView = elevatorView;
            _elevatorModel = elevatorModel;
            _workerModel = workerModel;
            _mineshaftMvcCollection = mineshaftMvcCollection;
        ...
        private Vector2 GetCurrentTargetPosition()
        {
            switch (_workerModel.State)
            {
                case WorkerState.LOAD:
                    return _mineshaftMvcCollection.GetView(_targetMineshaftNumber).Positions.ElevatorLoadPosition;
        ...
        private void TriggerWorking(bool working)
        {
            ...
            if (!working)
            {
                if(_workerModel.State == WorkerState.LOAD && (_workerModel.CarryingAmount.Value >= _workerModel.CarryingCapacity.Value || _targetMineshaftNumber >= _mineshaftMvcCollection.GetCount()))
                {
        ...
        {
            return _mineshaftMvcCollection.GetModel(_targetMineshaftNumber).DrawResource(amount);
        }
    }
}